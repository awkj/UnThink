 ...
 */
const TODAY_GROUP_HEADING_PREFIX = "today:group-header:"
const todayGroupHeadingId = (groupId: string) => `${TODAY_GROUP_HEADING_PREFIX}${groupId}`
...
export function groupTodayItems(items: (TaskInfo | ProjectInfoState)[], modelData: ITaskModelData): GroupedTodayItems {
  ...
  for (const item of items) {
    const key = item.type === "project" ? resolveProjectGroupKey(modelData, item) : resolveTaskGroupKey(modelData, item)
