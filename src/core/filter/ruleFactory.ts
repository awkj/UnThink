...
export function ruleFactory(schema: ItemSchema, options: RuleFactoryOptions = {}): (expr: string) => ParseResult {
...
function compileProgram(
  ...
  const statement = ast.type === "Program" ? ast.body[0] : undefined
  if (ast.type !== "Program" || ast.body.length !== 1 || statement?.type !== "ExpressionStatement") {
  ...
  return compileBoolean(statement.expression, fields, constants)
}
...
function compileBinary(
  ...
  const rightIsNull =
    (op === "===" || op === "!==") && node.right.type === "Literal" && (node.right as { value: unknown }).value === null
